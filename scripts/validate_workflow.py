...
def load_workflow_json(file_path: Path) -> Tuple[Optional[Dict[str, Any]], Optional[str]]:
    """
...
def validate_workflow_schema(workflow: Dict[str, Any], file_path: Path) -> List[str]:
    ...
    valid_top_level_keys = {"inputs", "tasks", "outputs", "description", "_designer", "metadata", "version", "name"}
    for key in workflow.keys():
        if key not in valid_top_level_keys:
            errors.append(f"Unknown top-level key: '{key}' (valid keys: {valid_top_level_keys})")
    ...
    if "tasks" in workflow:
        if not isinstance(workflow["tasks"], list):
            ...
            for idx, task in enumerate(workflow["tasks"]):
                ...
                valid_task_keys = {"name", "task", "inputs", "outputs", "dependencies", "cache"}
                ...
                if "inputs" in task and not isinstance(task["inputs"], dict):
                    errors.append(f"Task '{task.get('name', f'index_{idx}')}' 'inputs' must be a dictionary")
                ...
                if "cache" in task and not isinstance(task["cache"], bool):
                    errors.append(f"Task '{task.get('name', f'index_{idx}')}' 'cache' must be a boolean")
            ...
            if isinstance(input_value, str):
                ...
                if input_value.startswith("tasks."):
                    ...
                    if len(parts) >= 3:
                        ...
                        elif ref_task in task_outputs and ref_output not in task_outputs[ref_task]:
                            errors.append(
...
def validate_workflow_file(file_path: Path, project_root: Path) -> Tuple[bool, List[str]]:
    """

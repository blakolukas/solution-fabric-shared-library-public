...
def huggingface_login(token: str = None, add_to_git_credential: bool = False, new_session: bool = True):
    ...
    if token is None:
        ...
        if token is None:
            raise ValueError(
                "No HuggingFace token provided. " "Please set HF_TOKEN environment variable or pass token parameter."
            )

    # Login to HuggingFace Hub
    print("Logging in to HuggingFace Hub...")
    login(token=token, add_to_git_credential=add_to_git_credential, new_session=new_session)

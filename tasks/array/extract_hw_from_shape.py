...
@task(
    ...
    output_types={"height": "int", "width": "int"},
...
def extract_hw_from_shape(input_shape):
    ...
    if not isinstance(input_shape, (list, tuple)) or len(input_shape) < 4:
        raise ValueError(
            "'input_shape' must be a list/tuple with at least 4 elements (N,C,H,W)."
        )
    ...
    if isinstance(height, str) or isinstance(width, str):
        ...
        raise ValueError(
            f"Cannot extract concrete H/W from symbolic shape: {input_shape}"
        )
